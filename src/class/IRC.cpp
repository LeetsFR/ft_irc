...
void IRC::initSocket()
{
    ...
    if (bind(this->_fdSocket, (struct sockaddr *)&this->_addr, sizeof(this->_addr)))
        throw logic_error("Exception: Cannot bind the server to the address");
...
bool IRC::nicknameExist(std::string& nickname)
{
    ...
    for (size_t i = 0; i < nbClient; i++)
    {
        if (nickname.compare(this->_listClient[i]->getNickname()) == 0 ||    nickname.compare(this->_listClient[i]->getUniqId()) == 0)
            return (true);
    }     
    return (false);
}
